...
ここで, $D_{0, t} := \beta^t \frac{U_{c}(t)}{U_{c}(0)}$ は**確率的割引因子**（stochastic discount factor）です.
...
なぜなら, 自然利子率は $r_t^n := i_t^n - \mathbb{E}_t\left[\pi_{t+1}^n\right]$ で定義され, @eq-lin-nkis と @eq-lin-output-flexible を用いると,
...
NKIS @eq-nk-is の解釈としては, 産出ギャップが将来の産出ギャップ予想 $\mathbb{E}_t\left[\widetilde{Y}_{t+1}\right]$ と, 実質金利のギャップ $i_t - \mathbb{E}_t\left[\pi_{t+1}\right] - r_t^n$ によって決定されることを示しています.

1. **将来の産出ギャップ予想**: 家計は消費を平滑化しようとするため, 将来の産出ギャップが高いと予想される場合, 現在の産出ギャップも高くなる
1. **実質金利のギャップ**: 実質金利が高い場合, 貯蓄を増やし現在の消費を減らす. その結果, 産出ギャップが減少する
...
$$
\begin{aligned}
\pi_t&= \beta \mathbb{E}_t\left[\hat{\pi}_{t+1}\right] + (1-\beta)\pi +
...
$$
\hat{\pi}_{t} = \beta \mathbb{E}_t\left[\pi_{t+1}\right] + \frac{\varepsilon-1}{\varphi} \widehat{MC^r}_{t}.
$$

@lem-lin-mc を用いて,

$$
\hat{\pi}_{t} = \beta \mathbb{E}_t\left[\pi_{t+1}\right] + \frac{\varepsilon-1}{\varphi} (\sigma+\phi)\widetilde{Y}_t.
$$
...
1. **インフレ予想**: 急激な価格改訂はコストがかかるため, インフレが予想される場合は, 企業は事前に価格を引き上げる
1. **産出ギャップ**: 需要が高まると, 労働需要が高まり, 賃金が上昇する. 企業はコスト上昇を価格に転嫁し, インフレ率が上昇する
