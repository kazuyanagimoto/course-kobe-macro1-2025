...
$$
\begin{aligned}
\widetilde{Y}_t &= \mathbb{E}_t\left[\widetilde{Y}_{t+1}\right] - \frac{1}{\sigma}\left(i_t - r_t^n -\mathbb{E}_t\left[\pi_{t+1}\right]\right) & \text{(NK IS Curve)} \\
...
$$
\widetilde{Y}_t = \mathbb{E}_t\left[\widetilde{Y}_{t+1}\right] - \frac{1}{\sigma}\left(i_t - r_t^n - \mathbb{E}_t\left[\pi_{t+1}\right]\right)
$${#eq-nk-is}

ここで, 自然利子率 $r_t^n$ は以下で定義されます.

$$
r_t^n := -\log \beta + \frac{\sigma(1+\phi)}{\sigma + \phi}\left(\hat{A}_t - \mathbb{E}_t\left[\hat{A}_{t+1}\right]\right).
$$
